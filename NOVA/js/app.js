...
const state = {
  ...
  mobileLite: window.matchMedia(MOBILE_QUERY).matches,
  floatingNodes: [],
  lenis: null
};
...
function finishIntro() {
  ...
  qsa(".hero .reveal-item").forEach((item) => {
    item.style.opacity = "1";
    item.style.transform = "translateY(0)";
  });
  if (state.introFallbackTimer) {
    window.clearTimeout(state.introFallbackTimer);
    state.introFallbackTimer = null;
  }
}

function startIntroFallback(duration = 1800) {
  if (state.introFallbackTimer) {
    window.clearTimeout(state.introFallbackTimer);
  }
  state.introFallbackTimer = window.setTimeout(finishIntro, duration);
}
...
function animateFloatingObjects() {
  ...
  items.forEach((node, index) => {
    ...
    node.style.setProperty("--float-mobile-index", index);
  ...
  if (!frame || state.reduceMotion || state.mobileLite) {
    return;
  }
...
function setupRevealObserver() {
  ...
  const revealDistance = state.mobileLite ? 16 : 28;
  const revealDuration = state.mobileLite ? 560 : 860;
  const revealTarget = (target) => {
    target.dataset.revealed = "true";
    animate(target, {
      translateY: [revealDistance, 0],
      opacity: [0, 1],
  ...
  const observer = new IntersectionObserver((entries) => {
    ...
    rootMargin: state.mobileLite ? "0px 0px -4% 0px" : "0px 0px -8% 0px",
    threshold: state.mobileLite ? 0.08 : 0.16
  });
...
function setupCounters() {
  ...
  const run = () => {
    ...
    animate(qsa(".hero-metrics li"), {
      translateY: [state.mobileLite ? 7 : 10, 0],
      opacity: [0.86, 1],
      duration: state.mobileLite ? 360 : 520,
      delay: stagger(state.mobileLite ? 45 : 80),
      ease: "outExpo"
    });
  };
...
function applyLanguage(language, animateChange = false) {
  ...
  const targets = [".hero-copy", ".marquee-track", ".builder-layout", ".contact-layout"].join(",");
  const distance = state.mobileLite ? 5 : 8;
  const outDuration = state.mobileLite ? 120 : 140;
  const inDuration = state.mobileLite ? 300 : 340;
  ...
  window.setTimeout(safeUpdate, outDuration + 80);
...
function updateSequenceScene() {
  ...
  if (performance.now() < state.sequenceHoldUntil) {
    return;
  }
...
function setupSequenceControls() {
  ...
  qsa(".sequence-number").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      state.sequenceHoldUntil = performance.now() + 1200;
...
function syncMotionPreference() {
  ...
  const update = () => {
    ...
    state.mobileLite = mobileMedia.matches;
    document.body.classList.toggle("mobile-lite", state.mobileLite);
  };
...
function loop() {
  if (!state.reduceMotion && state.floatingNodes.length) {
    ...
    const driftScale = state.mobileLite ? 0.38 : 1;
    state.floatingNodes.forEach((item) => {
      const progress = now / item.speed + item.phase;
      const driftY = Math.sin(progress) * item.amplitudeY * driftScale;
      const driftRotate = Math.cos(progress * 0.92) * item.amplitudeRotate * driftScale;
